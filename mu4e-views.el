...
determine whether to filter or not."
  ...
  (let* ((html (mu4e-message-field msg :body-html))
         ...
         (inlines (if (mu4e-views-mu4e-ver->= '(1 10)) (plist-get msg :gnus-inline) attachments)))
    (unless (or html txt)
      (setq txt "")
      ...
      (with-temp-buffer
	    ...
	    (mapc (lambda (attachment)
                ...
                (let* ((fname (plist-get attachment :name))
                       ...
                       have-matched)
                  (mu4e-views-debug-log "Going to try to match [%s] for [%s] will store in %s"
                                        cid-search-str
                                        fname
                                        tmp-attachment-name)
                  (save-excursion
